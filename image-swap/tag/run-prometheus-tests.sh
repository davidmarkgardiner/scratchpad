...
query_prometheus() {
  ...
  local query=$(echo "$metric{job_name=\"$job_name\"}" | sed 's/ /%20/g')
  
  # Query Prometheus
  local result=$(curl -s "http://localhost:9090/api/v1/query?query=$query" | jq -r '.data.result[0].value[1]' 2>/dev/null)
