...
deploy_test_job() {
  ...
  # Delete the job first if it exists to avoid "field is immutable" errors
...
cleanup_jobs() {
  ...
  kubectl delete job image-push-job-test-success --ignore-not-found=true
  kubectl delete job image-push-job-test-failure --ignore-not-found=true
  kubectl delete job image-push-job-test-stuck --ignore-not-found=true
  kubectl delete job image-push-job-test-backoff --ignore-not-found=true
...
main() {
  ...
  # Deploy success job
  deploy_test_job "test-image-push-job.yaml"
  sleep 2
  check_job_status "image-push-job-test-success"
  
  # Deploy failure job
  deploy_test_job "test-image-push-job-failure.yaml"
  sleep 2
  check_job_status "image-push-job-test-failure"
  
  # Create a backoff limit job if it doesn't exist
  if [ ! -f "test-image-push-job-backoff.yaml" ]; then
    echo -e "${YELLOW}Creating backoff limit job definition...${NC}"
    cat > test-image-push-job-backoff.yaml << EOF
apiVersion: batch/v1
kind: Job
metadata:
...
EOF
  ...
  deploy_test_job "test-image-push-job-backoff.yaml"
  sleep 2
  check_job_status "image-push-job-test-backoff"
  
  # Deploy stuck job
  deploy_test_job "test-image-push-job-stuck.yaml"
  sleep 2
  ...
  echo "  ./run-prometheus-tests-fixed.sh cleanup"
}
