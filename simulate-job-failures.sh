...
# Check Prometheus metrics if possible
echo
...
if command -v jq &> /dev/null; then
  ...
  if curl -s http://localhost:9090/-/healthy > /dev/null; then
    ...
    # Query for failed jobs count
    QUERY="count(kube_job_status_failed{namespace=\"$NAMESPACE\",job_name=~\"image-push-job-test-failure-.*\"})"
    RESULT=$(curl -G -s "http://localhost:9090/api/v1/query" --data-urlencode "query=$QUERY" | jq -r '.data.result[0].value[1]' 2>/dev/null)
    
    if [ -z "$RESULT" ] || [ "$RESULT" == "null" ]; then
      echo -e "${RED}✗ No data found for failed jobs in Prometheus${NC}"
    else
      echo -e "${GREEN}✓ Failed jobs count: $RESULT${NC}"
    fi
    
    # Kill the port-forward process
    kill $PF_PID 2>/dev/null
  else
    echo -e "${RED}✗ Cannot access Prometheus${NC}"
  fi
else
  echo -e "${RED}✗ jq is required for Prometheus queries${NC}"
fi
